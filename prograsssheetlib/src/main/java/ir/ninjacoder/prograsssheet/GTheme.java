...
public class GTheme {
    ...
    private static void saveToSharedPreferences(
        ...
        if (new File(wallpaperPath).getName().startsWith("preview")) {
            ...
            SharedPreferences sharedPreferences = context.getSharedPreferences("getvb", Context.MODE_PRIVATE);
        ...
        if (ghostFile != null) {
            SharedPreferences thememanagersoft = context.getSharedPreferences("thememanagersoft", Context.MODE_PRIVATE);
            thememanagersoft.edit()
                    .putString("themes", ghostFile.getAbsolutePath())
                    .apply();
    ...
    private static boolean isValidThemeFolder(File folder) {
        String[] validExtensions = {".ghost", ".jpg", ".png", ".webp", ".gif"};
        File[] files = folder.listFiles();
