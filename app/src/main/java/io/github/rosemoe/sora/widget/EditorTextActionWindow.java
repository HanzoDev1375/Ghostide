...
public class EditorTextActionWindow extends EditorPopupWindow
            ...
            adptor =
                    new TextActionAd(
                            ...
                            new TextActionAd.OnItemClick() {
                                ...
                                @Override
                                public void onItemClickChange(
                                        int posNow, View myview, ImageView img) {
                                    View v = myview;
                                    switch (posNow) {
                                        case 0:
                                            {
                                        ...
                                        case 2:
                                            {
                                                editor.copyText();
                                                img.setEnabled(
                                                        editor.hasClip() && editor.isEditable());
                                                dismiss();
                                                break;
                                            }
                                        case 3:
                                        ...
                                        case 7:
                                            {
                                                if (editor.getEditorLanguage()
                                                    ...
                                                    toolss.run(v.getContext(), editor, v);
                                                        ...
                                                        instanceof KotlinLanguage) {
                                                    KotlinTools tools = new KotlinTools();
                                                    tools.run(v.getContext(), editor, v);
                                                } else if (editor.getEditorLanguage()
                                                        instanceof SMLang) {
                                                    new ToolItem()
                                                            .bindSmail(v.getContext(), editor, v);
                                                } else if (editor.getEditorLanguage()
                                                        instanceof JsonLanguage) {
                                                    JsonTools.run(
                                                            (IdeEditor) editor, v, v.getContext());
                                                } else if (editor.getEditorLanguage()
                                                        instanceof XMLLanguage) {
                                                    new ToolItem()
                                                            .bindXml(v.getContext(), editor, v);
                                                } else if (editor.getEditorLanguage()
                                                        instanceof GroovyLanguage) {
                                                    var gr =
                                                            new GradleTools(
                                                                    v.getContext(),
                                                                    (IdeEditor) editor,
    ...
    private void updateBtnState() {
        ...
        setSize(
                Math.min(bin.getRoot().getMeasuredWidth(), (int) (mEditor.getDpUnit() * 230)),
                getHeight());
    }
    ...
    private boolean isShow() {
        InputMethodManager imm =
                (InputMethodManager)
                        mEditor.getContext().getSystemService(Context.INPUT_METHOD_SERVICE);
