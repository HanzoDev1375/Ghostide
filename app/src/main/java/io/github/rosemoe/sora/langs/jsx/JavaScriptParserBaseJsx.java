 ...
 */
public abstract class JavaScriptParserBaseJsx extends Parser {

  // 2020/10/27 for jsx
  private final Deque<String> _tagNames = new ArrayDeque<>();

  public JavaScriptParserBaseJsx(TokenStream input) {
    super(input);
  }
  ...
  protected boolean notLineTerminator() {
    return !here(JavaScriptParserJsx.LineTerminator);
  }

  protected boolean notOpenBraceAndNotFunction() {
    int nextTokenType = _input.LT(1).getType();
    return nextTokenType != JavaScriptParserJsx.OpenBrace
        && nextTokenType != JavaScriptParserJsx.Function_;
  }

  protected boolean closeBrace() {
    return _input.LT(1).getType() == JavaScriptParserJsx.CloseBrace;
  }
  ...
  protected boolean lineTerminatorAhead() {
    ...
    if (ahead.getType() == JavaScriptParserJsx.LineTerminator) {
      // There is definitely a line terminator ahead.
      return true;
    }
    if (ahead.getType() == JavaScriptParserJsx.WhiteSpaces) {
    ...
    return (type == JavaScriptParserJsx.MultiLineComment
            && (text.contains("\r") || text.contains("\n")))
        || (type == JavaScriptParserJsx.LineTerminator);
  }
