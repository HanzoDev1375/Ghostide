 ...
 *
 * <p><b>AndroidIDE Changed</b>: Allow to choose if the status bar and navigation bar's color should
 * be changed or not.
 *
 * @see <a
...
public class EdgeToEdgeUtils {
  ...
  public static final SystemBarStyle DEFAULT_STATUS_BAR_STYLE =
      SystemBarStyle.auto(TRANSPARENT, TRANSPARENT);
  public static final SystemBarStyle DEFAULT_NAVIGATION_BAR_STYLE =
      SystemBarStyle.auto(DEFAULT_LIGHT_SCRIM, DEFAULT_DARK_SCRIM);

  private EdgeToEdgeUtils() {}

  /**
   * Enable edge-to-edge mode for the provided activity, using the built-in {@link
   * EdgeToEdge#enable(androidx.activity.ComponentActivity, SystemBarStyle, SystemBarStyle)}.
   *
  ...
  public static void applyEdgeToEdgeManually(
    ...
    applyEdgeToEdgeManually(window, edgeToEdgeEnabled, null, null, statusBarColor, navBarColor);
  }
   ...
   *     colors on status bars. {@code null} to use the default color from {@code
   *     ?android:attr/colorBackground}.
   * @param navigationBarOverlapBackgroundColor The reference background color to decide the icon
   *     colors on navigation bars.{@code null} to use the default color from {@code
   *     ?android:attr/colorBackground}.
   */
  public static void applyEdgeToEdgeManually(
      @NonNull Window window,
    ...
    setLightStatusBar(
        window, isUsingLightSystemBar(color, isColorLight(statusBarOverlapBackgroundColor)));
    ...
    setLightNavigationBar(
        window, isUsingLightSystemBar(color, isColorLight(navigationBarOverlapBackgroundColor)));
  }
