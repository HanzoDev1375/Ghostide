...
import ir.ninjacoder.ghostide.marco.BindViewListMarger;
import ir.ninjacoder.ghostide.marco.FileCounter;
import ir.ninjacoder.ghostide.marco.binder.bindchilder.ChilderRecyclerviewCompat;
    ...
    if (FileUtil.isDirectory(token)) {
      ...
      if (token.endsWith(".html")) {
        ...
        try {
          ...
          GlideCompat.loadImgPdf(file.toString(), imageview1);
        } catch (Exception err) {
          err.printStackTrace();
        }
