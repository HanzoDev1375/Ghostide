...
public class CustomImageDataFetcher implements DataFetcher<Drawable> {
  ...
  private Drawable resolveImage(JavaAnalysisResult result) {
    if (result.isInterface()) {
      return ContextCompat.getDrawable(context, R.drawable.musicerror);
    } else if (result.isClass()) {
      return ContextCompat.getDrawable(context, R.drawable.musicnext);
    } else if (result.isAbstract()) {
      return ContextCompat.getDrawable(context, R.drawable.musicplay);
    }else if(result.isEnum() ) {
    	return ContextCompat.getDrawable(context, R.drawable.musicstop);
    } 
