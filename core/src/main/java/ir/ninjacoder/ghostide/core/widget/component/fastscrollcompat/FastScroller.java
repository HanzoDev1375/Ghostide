...
package ir.ninjacoder.ghostide.core.widget.component.fastscrollcompat;
...
import ir.ninjacoder.ghostide.core.widget.component.fastscrollcompat.Predicate;
...
public class FastScroller {
  ...
  @NonNull private final ViewHelper mViewHelper;
  @NonNull private final AnimationHelper mAnimationHelper;
  private final int mTrackWidth;
  private final int mThumbWidth;
  private final int mThumbHeight;
  @NonNull private final View mTrackView;
  @NonNull private final View mThumbView;
  @NonNull private final TextView mPopupView;
  @NonNull private final Rect mTempRect = new Rect();
  @Nullable private Rect mUserPadding;
  ...
  @NonNull private final Runnable mAutoHideScrollbarRunnable = this::autoHideScrollbar;

  public FastScroller(
      @NonNull ViewGroup view,
      @NonNull ViewHelper viewHelper,
      @Nullable Rect padding,
      @NonNull Drawable trackDrawable,
      @NonNull Drawable thumbDrawable,
      @NonNull Consumer<TextView> popupStyle,
      @NonNull AnimationHelper animationHelper) {

    mMinTouchTargetSize =
        view.getResources().getDimensionPixelSize(R.dimen.afs_min_touch_target_size);
    ...
    mTrackWidth =
        requireNonNegative(
            trackDrawable.getIntrinsicWidth(), "trackDrawable.getIntrinsicWidth() < 0");
    mThumbWidth =
        requireNonNegative(
            thumbDrawable.getIntrinsicWidth(), "thumbDrawable.getIntrinsicWidth() < 0");
    mThumbHeight =
        requireNonNegative(
            thumbDrawable.getIntrinsicHeight(), "thumbDrawable.getIntrinsicHeight() < 0");
  ...
  public void setPadding(int left, int top, int right, int bottom) {
    if (mUserPadding != null
        && mUserPadding.left == left
        && mUserPadding.top == top
        && mUserPadding.right == right
        && mUserPadding.bottom == bottom) {
      return;
    }
    if (mUserPadding == null) {
  ...
  private void onPreDraw() {
    ...
    if (hasPopup) {
      FrameLayout.LayoutParams popupLayoutParams =
          (FrameLayout.LayoutParams) mPopupView.getLayoutParams();
        ...
        case Gravity.CENTER_VERTICAL:
          {
            ...
            thumbAnchorY =
                thumbPaddingTop
                    + (mThumbHeight - thumbPaddingTop - mThumbView.getPaddingBottom()) / 2;
            break;
          }
        case Gravity.BOTTOM:
      ...
      layoutView(mPopupView, popupLeft, popupTop, popupLeft + popupWidth, popupTop + popupHeight);
    }
  }
  ...
  private void updateScrollbarState() {
    ...
    mThumbOffset =
        mScrollbarEnabled
            ? (int)
                ((long) getThumbOffsetRange() * mViewHelper.getScrollOffset() / scrollOffsetRange)
            : 0;
  }
  ...
  private boolean onTouchEvent(@NonNull MotionEvent event) {
    ...
    switch (event.getAction()) {
      ...
      case MotionEvent.ACTION_MOVE:
        if (!mDragging
            && isInViewTouchTarget(mTrackView, mDownX, mDownY)
  ...
  private boolean isInView(@NonNull View view, float x, float y) {
    int scrollX = mView.getScrollX();
    int scrollY = mView.getScrollY();
    return x >= view.getLeft() - scrollX
        && x < view.getRight() - scrollX
        && y >= view.getTop() - scrollY
  ...
  private boolean isInViewTouchTarget(@NonNull View view, float x, float y) {
    ...
    return isInTouchTarget(
            x, view.getLeft() - scrollX, view.getRight() - scrollX, 0, mView.getWidth())
        && isInTouchTarget(
            y, view.getTop() - scrollY, view.getBottom() - scrollY, 0, mView.getHeight());
  }

  private boolean isInTouchTarget(
      float position, int viewStart, int viewEnd, int parentStart, int parentEnd) {
  ...
  private void postAutoHideScrollbar() {
    ...
    if (mAnimationHelper.isScrollbarAutoHideEnabled()) {
      mView.postDelayed(
          mAutoHideScrollbarRunnable, mAnimationHelper.getScrollbarAutoHideDelayMillis());
    }
  }
