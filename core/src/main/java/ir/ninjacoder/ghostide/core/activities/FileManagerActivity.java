...
public class FileManagerActivity extends BaseCompat
    ...
    private void initialize(Bundle _savedInstanceState) {
        ...
        var helper = new RecyclerViewHelper(
        bind.recyclerview2,
    ...
    private void setupSearchModeMenu() {
        bind.searchbar.setAvatarOnClickListener(v -> {
            PopupMenu popup = new PopupMenu(this, v);

            MenuItem item1 = popup.getMenu().add(0, 1, 0, getString(R.string.search_normal));
            MenuItem item2 = popup.getMenu().add(0, 2, 0, getString(R.string.search_regex));
            MenuItem item3 = popup.getMenu().add(0, 3, 0, getString(R.string.search_regex_case));
            MenuItem item4 = popup.getMenu().add(0, 4, 0, getString(R.string.search_regex_multiline));
            MenuItem item5 = popup.getMenu().add(0, 5, 0, getString(R.string.search_regex_dotall));
            FileManagerAd.SearchMode currentMode = fileListItem.getCurrentSearchMode();
            switch (currentMode) {
                case NORMAL -> item1.setChecked(true);
                case REGEX -> item2.setChecked(true);
                case REGEX_CASE_INSENSITIVE -> item3.setChecked(true);
                case REGEX_MULTILINE -> item4.setChecked(true);
                case REGEX_DOTALL -> item5.setChecked(true);
            }

            popup.setGroupCheckable(0, true, true);

            popup.setOnMenuItemClickListener(item -> {
                item.setChecked(true);

                fileListItem.setSearchMode(switch (item.getItemId()) {
                    case 1 -> FileManagerAd.SearchMode.NORMAL;
                    case 2 -> FileManagerAd.SearchMode.REGEX;
                    case 3 -> FileManagerAd.SearchMode.REGEX_CASE_INSENSITIVE;
                    case 4 -> FileManagerAd.SearchMode.REGEX_MULTILINE;
                    case 5 -> FileManagerAd.SearchMode.REGEX_DOTALL;
                    default -> FileManagerAd.SearchMode.NORMAL;
                });

                if (!TextUtils.isEmpty(bind.searchbar.getText())) {
                    fileListItem.filter(bind.searchbar.getText().toString());
                }
                return true;
            });
    ...
    void FolderMaker() {
        var dialog = new GhostWebMaterialDialog(FileManagerActivity.this)
    ...
    void setRenameFile(int _pos) {
        ...
        AlertDialog dialog = new GhostWebMaterialDialog(FileManagerActivity.this)
    ...
    private void setJarDecompiler() {
        androidx.appcompat.app.AlertDialog dialog = new GhostWebMaterialDialog(FileManagerActivity.this)
                                ...
                                if (output.getText().toString().isEmpty()) {
                                    ...
                                    new AsyncTask<String, String, String>() {
                                        ...
                                        protected String doInBackground(String... params) {
                                            ...
                                            try {
                                                org.benf.cfr.reader.Main.main(pv.toArray(new String
                                                        [0]));
                                            } catch (Exception e) {
                                                runOnUiThread(
                                                new Runnable() {
        ...
        sheet.setOnItemClickLabe(
                pos -> {
                    switch (pos) {
                        ...
                        case 2:
                            {
                                SharedPreferences themePrefs = getSharedPreferences("thememanagersoft", MODE_PRIVATE);
                    ...
                    () -> {
                        if (FileUtil.isDirectory(fileModels.get(_number).getFilePath())) {
                            try {
                                ...
                                String outputFilePath = Folder.concat("/")
                                        .concat(
    ...
    void setBackupTheme() {
        if (FileUtil.isFile("/storage/emulated/0/GhostWebIDE/theme/theme.AA")) {
            ...
            try {
                var zip4 = new net.lingala.zip4j.ZipFile(
                new File(
    ...
    void appUpdate() {
        var updateversion = new UpadteAppView(
        this,
        bind.downloder,
        bind.fabAdd,
        () -> {
        ...
        sh1.setOnItemClickLabe(
                (pos333) -> {
                    switch (pos333) {
                        ...
                        case 6:
                            {
                                ...
                                ViewGroup viewGroup = findViewById(android.R.id.content);
                                View dialogview1 = getLayoutInflater().inflate(R.layout.makefolder, viewGroup, false);
                        ...
                        case 10:
                            {
                                var jsonToJava = new GsonToClass(
                                FileManagerActivity.this,
                                Folder,
                                () -> {
                                    reLoadFile();
                        ...
                        case 11:
                            {
                                var nodeChild = new ClassNodePaserImpl(
                                Folder,
                                FileManagerActivity.this,
                                () -> {
                                    reLoadFile();
        ...
        sheet.setOnItemClickLabe(
                (pos_) -> {
                    switch (pos_) {
                        ...
                        case 8:
                            {
                                var plugin = new PluginCompressorPgb(
                                new PluginextractorFace() {
