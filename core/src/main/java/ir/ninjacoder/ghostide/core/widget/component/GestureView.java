...
public class GestureView extends FrameLayout implements IGestureComponent {
  ...
  void init() {
    ...
    LayoutInflater.from(getContext())
        .inflate(R.layout.dkplayer_layout_gesture_control_view, this, true);
    ...
    ObjectUtils.setTextColor(mTextPercent);
    ObjectUtils.setColorFilter(mIcon);
    bars = new SquigglyProgress();
    bars.setWaveLength(100);
    bars.setLineAmplitude(8);
    bars.setPhaseSpeed(25);
    if (bars != null) bars.setStrokeWidth(convertToPx(getContext(), 4));
    bars.setTransitionEnabled(true);
    bars.setAnimate(true);
    mProgressPercent.setProgressDrawable(bars);
  }
  ...
  public void onPositionChange(int slidePosition, int currentPosition, int duration) {
    ...
    mTextPercent.setText(
        String.format("%s/%s", stringForTime(slidePosition), stringForTime(duration)));
  }
