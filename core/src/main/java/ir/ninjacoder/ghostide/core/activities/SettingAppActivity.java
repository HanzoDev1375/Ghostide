...
import ir.ninjacoder.ghostide.core.adapter.ListAppIconAd;
import ir.ninjacoder.ghostide.core.config.AppIconManager;
import ir.ninjacoder.ghostide.core.config.PrfnsUtil;
import ir.ninjacoder.ghostide.core.databinding.LayoutEditorSizeBlurBinding;
import ir.ninjacoder.ghostide.core.databinding.Mp2Binding;
import ir.ninjacoder.ghostide.core.enums.EffectTypeManager;
import ir.ninjacoder.ghostide.core.enums.ErudaThemeManager;
import ir.ninjacoder.ghostide.core.enums.ErudaThemes;
import ir.ninjacoder.ghostide.core.filehelper.IconCursorImpl;
...
public class SettingAppActivity extends BaseCompat {
    ...
    private void initialize(Bundle _savedInstanceState) {
        ...
        blurmodel = findViewById(R.id.blurmodel);
        ...
        // impl soon
        blurmodel.setTitle("BlurMod");
        blurmodel.setDescription("Enable BlurMod Mode in the entire Views by activating this feature");
        ...
        blurmodel.setSwitchChangedListener(
                (vv, is) -> {
                    if (is) {
                        thememanagersoft.edit().putBoolean("blur", true).apply();
                    } else {
                        thememanagersoft.edit().remove("blur").apply();
                    }
                });
        ...
        breaklevelmodel.setSwitchChangedListener(
                (bin, is) -> {
                    if (is)
                        GhostIdeAppLoader.getPrefManager().edit().putBoolean("breaks", true).apply();
    ...
    void _seechackswich() {
        ...
        if (setfont.getBoolean("blur", false) == true) {
            minimap.setValue(true);
        } else minimap.setValue(false);
        ...
        list.setOnItemClickListener(
                new AdapterView.OnItemClickListener() {
                    ...
                    public void onItemClick(AdapterView<
                                    ?> _param1, View _param2, int _param3, long _param4) {
    ...
    void showEF() {
        ...
        new MaterialAlertDialogBuilder(this)
                ...
                .setSingleChoiceItems(
                        ...
                        currentIndex,
                        (dialog, which) -> {
                            PowerModeEffectManager.EffectType selectedEffect = PowerModeEffectManager.EffectType.values()[
                            which];
