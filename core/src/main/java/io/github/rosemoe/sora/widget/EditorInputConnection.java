...
class EditorInputConnection extends BaseInputConnection {
  ...
  protected void commitTextInternal(CharSequence text, boolean applyAutoIndent) {
    ...
    if (replacement == null
      ...
      if (getCursor().isSelected()
        ...
        mEditor
            .getText()
            .insert(getCursor().getLeftLine(), getCursor().getLeftColumn(), autoSurroundPair[0]);
        // insert right
        mEditor
            .getText()
            .insert(getCursor().getRightLine(), getCursor().getRightColumn(), autoSurroundPair[1]);
  ...
  public boolean deleteSurroundingText(int beforeLength, int afterLength) {
    ...
    int composingStart =
        composing ? getCursor().getIndexer().getCharIndex(mComposingLine, mComposingStart) : 0;
    int composingEnd =
        composing ? getCursor().getIndexer().getCharIndex(mComposingLine, mComposingEnd) : 0;
