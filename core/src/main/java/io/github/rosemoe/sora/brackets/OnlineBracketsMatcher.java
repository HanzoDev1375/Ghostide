 ...
 */
package io.github.rosemoe.sora.brackets;
...
public class OnlineBracketsMatcher implements BracketsProvider {
  ...
  private io.github.rosemoe.sora.lang.brackets.PairedBracket tryComputePaired(Content text, int index) {
    ...
    if (symbolIndex != -1) {
      ...
      if ((symbolIndex & 1) == 0) {
        ...
        for (int i = index + 1; i < text.length() && i - index < limit; i++) {
          ...
          if (ch == b) {
            if (stack <= 0) {
              return new io.github.rosemoe.sora.lang.brackets.PairedBracket(index, i);
            } else {
              stack--;
            }
          } else if (ch == a) {
        ...
        for (int i = index - 1; i >= 0 && index - i < limit; i--) {
          ...
          if (ch == b) {
            if (stack <= 0) {
              return new io.github.rosemoe.sora.lang.brackets.PairedBracket(i, index);
            } else {
              stack--;
            }
          } else if (ch == a) {
  ...
  @Override
  public io.github.rosemoe.sora.lang.brackets.PairedBracket getPairedBracketAt(Content text, int index) {
