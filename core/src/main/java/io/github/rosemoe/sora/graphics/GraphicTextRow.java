...
import static io.github.rosemoe.sora.text.TextStyle.isShadowLine;
import static io.github.rosemoe.sora.text.TextStyle.isStrikeThrough;
import android.text.*;
...
public class GraphicTextRow {
                ...
                for (int i = regionStart; i < regionEnd; i++) {
                    if (chars[i] == '\t') {
                        ...
                        if (lastStart != i) {
                            ...
                            currentPosition +=
                                    mPaint.measureTextRunAdvance(chars, lastStart, idx, regionStart, regionEnd);
                            if (idx < i) {
                                res = idx;
                                break;
                            } else {
    ...
    private float measureTextInner(int start, int end, float[] widths) {
        ...
        // استفاده از BidiFormatter برای تشخیص جهت متن
        String textSegment = new String(mText.value, start, end - start);
        BidiFormatter bidiFormatter = BidiFormatter.getInstance();
        boolean isRtl = bidiFormatter.isRtl(textSegment);

        // Can be called directly
        float width = mPaint.getTextRunAdvances(
                mText.value,
                start,
                end - start,
                start,
                end - start,
                isRtl, // استفاده از تشخیص جهت واقعی
                widths,
                widths == null ? 0 : start);
