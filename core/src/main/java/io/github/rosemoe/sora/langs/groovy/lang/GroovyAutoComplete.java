...
public class GroovyAutoComplete implements AutoCompleteProvider {
    ...
    public List<CompletionItem> getAutoCompleteItems(String prefix, TextAnalyzeResult analyzeResult, int line, int column) {
        item = new ArrayList<>();
        for (var ir1 : createArtifacts()) {
            if (ir1.startsWith(prefix)) {
                item.add(CppAsCompletion(ir1, "Artifacts"));
            }
        }
        for (var ir2 : createConfigs()) {
            if (ir2.startsWith(prefix)) {
                item.add(CppAsCompletion(ir2, "Configs"));
            }
        }
        for (var ir3 : createOtherCompletions()) {
            if (ir3.startsWith(prefix)) {
                item.add(CppAsCompletion(ir3, "OtherCompletions"));
            }
        }
        return item;
    }
