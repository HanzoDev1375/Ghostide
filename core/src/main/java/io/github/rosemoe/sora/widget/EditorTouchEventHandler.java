...
public class EditorTouchEventHandler
    implements GestureDetector.OnGestureListener,
        GestureDetector.OnDoubleTapListener,
        ScaleGestureDetector.OnScaleGestureListener {

  private static final int HIDE_DELAY = 3000;
  private static final int SELECTION_HANDLE_RESIZE_DELAY = 180;
  ...
  public EditorTouchEventHandler(CodeEditor editor) {
    ...
    minSize =
        TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_SP, 6, Resources.getSystem().getDisplayMetrics());
      ...
      case MotionEvent.ACTION_DOWN:
        {
          ...
          if (shouldDrawInsertHandle()
              && mEditor.getInsertHandleDescriptor().position.contains(e.getX(), e.getY())) {
  ...
  public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
    ...
    // If we do not finish it here, it can produce a high speed and cause the final scroll range to
    // be broken, even a NaN for velocity
    mScroller.forceFinished(true);
    mScroller.fling(
        mScroller.getCurrX(),
        mScroller.getCurrY(),
        ...
        mEditor.isOverScrollEnabled() && !mEditor.isWordwrap()
    ...
    try {
      mEditor.dispatchEvent(
          new DoubleClickEvent(
              mEditor, mEditor.getText().getIndexer().getCharPosition(line, column), e));
    } catch (Exception err) {
      err.printStackTrace();
    }
  ...
  class SelectionHandle {
    ...
    private boolean checkNoIntersection(
        SelectionHandleStyle.HandleDescriptor one, SelectionHandleStyle.HandleDescriptor another) {
    ...
    public void applyPosition(MotionEvent e) {
      ...
      var anotherDesc =
          type == LEFT ? mEditor.getRightHandleDescriptor() : mEditor.getLeftHandleDescriptor();
      float targetX =
          mScroller.getCurrX()
              + e.getX()
      ...
      if (line >= 0 && line < mEditor.getLineCount()) {
        ...
        int lastLine =
            type == RIGHT ? mEditor.getCursor().getRightLine() : mEditor.getCursor().getLeftLine();
        int lastColumn =
            type == RIGHT
