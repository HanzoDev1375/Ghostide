 ...
 * project in GitHub: https://github.com/Rosemoe/CodeEditor
 *
...
public class CodeEditor extends View
  ...
  /**
   * Draw whitespace characters after line content end <strong>Whitespace here only means space and
   * tab</strong>
   *
   * @see #setNonPrintablePaintingFlags(int)
   */
  ...
  private RectF miniMapRect = new RectF();
  private RectF miniMapViewport = new RectF();
  private int miniMapLineHeight = 3;
  private boolean miniMapDirty = true;
  private boolean miniMapShowingLineInfo = false;
  private long miniMapInfoHideTime = 0;
  private int miniMapCurrentLine = 0;
  private float miniMapInfoX = 0, miniMapInfoY = 0;
  private boolean isHoldingMiniMap = false;
  ...
  // فعال/غیرفعال کردن Sticky Scroll
  public void setStickyScrollEnabled(boolean enabled) {
    this.stickyScrollEnabled = enabled;
    invalidate();
  }
  ...
  private List<BlockLine> getStickyBlocks() {
    ...
    List<BlockLine> result = new ArrayList<>();

    for (BlockLine block : blocks) {
      if (block.startLine < firstVisibleLine && block.endLine >= firstVisibleLine) {
        result.add(block);
      }
    }
  ...
  private void drawStickyLineText(Canvas canvas, int line, float offsetX, float offsetY) {
    ...
    contentLine.timestamp = 0;
    prepareLine(line);
    int columnCount = mText.getColumnCount(line);

    List<Span> spans = getSpansForLine(line);
    if (spans == null || spans.isEmpty()) {
      spans = defSpans;
    }

    GraphicTextRow row = GraphicTextRow.obtain();
    row.set(mBuffer, 0, columnCount, getTabWidth(), spans, mPaint);
    ...
    while (columnCount > span.column && spanOffset < spans.size()) {
      ...
      float width = row.measureText(span.column, spanEnd);
  ...
  public void analyze(boolean runBgAnalyzer) {
    if (mSpanner != null) {
      mSpanner.analyze(mText, runBgAnalyzer);
    }
  }
  ...
  private float getRowTopForLine(int line) {
    RowIterator iterator = mLayout.obtainRowIterator(0);
    int currentRow = 0;
    float lastTop = 0;
    ...
    while (iterator.hasNext()) {
      ...
      if (row.lineIndex == line) {
        iterator.reset();
        return getRowTop(currentRow);
      }
      currentRow++;
      lastTop = getRowTop(currentRow);
    }
    iterator.reset();
    return lastTop;
  }

  private void resetMiniMapHold() {
    isHoldingMiniMap = false;
  }

  private int getMiniMapLineColor(int line) {
    List<Span> spans = getSpansForLine(line);
    if (spans != null && !spans.isEmpty()) {
      for (Span span : spans) {
        int colorId = span.getForegroundColorId();
        if (colorId != 0 && colorId != EditorColorScheme.TEXT_NORMAL) {
          return mColors.getColor(colorId);
        }
      }
    }

    if (mText.getColumnCount(line) == 0) {
      return mColors.getColor(EditorColorScheme.LINE_NUMBER);
    }
  ...
  public void deleteText() {
    ...
    if (cur.isSelected()) {
      mText.delete(
          cur.getLeftLine(), cur.getLeftColumn(), cur.getRightLine(), cur.getRightColumn());
    } else {
  ...
  public void insertText(String text, int selectionOffset) {
    ...
    if (selectionOffset != text.length()) {
      var pos =
          mText.getIndexer().getCharPosition(cur.getRight() - (text.length() - selectionOffset));
  ...
  public void setCommentJava() {
    helper.MakeCommentJava();
  }

  public void setCommentHtml() {
    helper.AutoRun();
  }
  ...
  /**
   * Set whether the editor should show a magnifier window when the user is moving the selection
   * handle
   */
  ...
  /**
   * Set the editor's language. A language is a for auto-completion,highlight and auto indent
   * analysis.
   *
  ...
  /**
   * Whether display the line number panel beside vertical scroll bar when the scroll bar is touched
   * by user
   *
  ...
  void drawView(Canvas canvas) {
    ...
    MutableInt firstLn =
        isFirstLineNumberAlwaysVisible() && isWordwrap() ? new MutableInt(-1) : null;

    drawRows(
        canvas, textOffset, postDrawLineNumbers, postDrawCursor, postDrawCurrentLines, firstLn);
    ...
    if (lineNumberNotPinned) {
      drawLineNumberBackground(
          canvas,
          offsetX,
          lineNumberWidth + mDividerMargin,
      ...
      for (int i = 0; i < postDrawCurrentLines.size(); i++) {
        drawRowBackground(
            ...
            (int) (textOffset - mDividerMargin));
      }
      drawDivider(
          canvas,
          offsetX + lineNumberWidth + mDividerMargin,
      ...
      if (firstLn != null && firstLn.value != -1) {
        ...
        if (postDrawLineNumbers.size() == 0
          ...
          y =
              (getRowBottom(row - 1) + getRowTop(row - 1)) / 2f
                  - (mLineNumberMetrics.descent - mLineNumberMetrics.ascent) / 2f
                  - mLineNumberMetrics.ascent
                  - getOffsetY();
        }
        ...
        switch (mLineNumberAlign) {
          ...
          case RIGHT:
            canvas.drawText(
                Integer.toString(firstLn.value + 1), offsetX + lineNumberWidth, y, mPaintOther);
            break;
          case CENTER:
            canvas.drawText(
                Integer.toString(firstLn.value + 1),
                offsetX + (lineNumberWidth + mDividerMargin) / 2f,
                y,
                mPaintOther);
        }
      }
    ...
    if (isStickyScrollEnabled()) {
      ...
      if (stickyBlocks != null && !stickyBlocks.isEmpty()) {
        ...
        drawStickyLines(canvas, stickyBlocks, offsetX1);
      }
    }
   ...
   * memory usage, but the editor can display text much quick. However, only when hardware
      ...
      if (span.problemFlags > 0
        ...
        if (color != 0 && span.column >= 0 && spanEnd - span.column >= 0) {
          ...
          for (int i = 0; i < waveCount; i++) {
            mPath.quadTo(
                waveLength * i + waveLength / 4, amplitude, waveLength * i + waveLength / 2, 0);
            mPath.quadTo(
  ...
  protected void drawRows(
      ...
      if (mCursor.isSelected() && line >= mCursor.getLeftLine() && line <= mCursor.getRightLine()) {
        ...
        if (mText.getColumnCount(line) == 0 && line != mCursor.getRightLine()) {
          ...
          drawRowRegionBackground(
              ...
              selectionStart,
              selectionEnd,
              mColors.getColor(EditorColorScheme.SELECTED_TEXT_BACKGROUND),
              line);
        }
      }
    ...
    for (int row = firstVis; row <= getLastVisibleRow() && rowIterator.hasNext(); row++) {
      ...
      if (!mHardwareAccAllowed
              ...
              if (isWordwrap()) {
                lineWidth =
                    measureText(
                            ...
                            spanEnd - Math.max(firstVisibleChar, span.column),
                            line)
                        + phi;
              } else {
        ...
        while (lastVisibleChar > span.column) {
          ...
          if ((span.problemFlags & Span.FLAG_DEPRECATED) != 0
            ...
            canvas.drawLine(
                paintingOffset,
                getRowTop(row) + getRowHeight() / 2f - getOffsetY(),
                paintingOffset + width,
                getRowTop(row) + getRowHeight() / 2f - getOffsetY(),
                mPaintOther);
          }
          ...
          if (span.problemFlags > 0
            ...
            if (color != 0 && span.column >= 0 && spanEnd - span.column >= 0) {
              ...
              if (isWordwrap()) {
                ...
                startOffset =
                    measureTextRegionOffset()
                        + measureText(mBuffer, 0, span.column, line)
                        - getOffsetX();
                lineWidth = measureText(mBuffer, span.column, spanEnd - span.column, line) + phi;
              }
        ...
        paintingOffset =
            offset + mRenderer.drawLineHardwareAccelerated(canvas, line, offset) - mDpUnit * 20;
      ...
      if (circleRadius != 0f
        ...
        if ((mNonPrintableOptions & FLAG_DRAW_WHITESPACE_IN_SELECTION) != 0
            && mCursor.isSelected()
      ...
      if (line == mConnection.mComposingLine) {
        ...
        if (paintStart != paintEnd) {
          ...
          mRect.left =
              paintingOffset
                  + measureText(mBuffer, firstVisibleChar, paintStart - firstVisibleChar, line);
    ...
    if (paintStart < paintEnd) {
      ...
      while (paintStart < paintEnd) {
        ...
        if (paintLine) {
          ...
          canvas.drawLine(
              offset + delta, rowCenter, offset + charWidth - delta, rowCenter, mPaintOther);
        }
  ...
  /**
   * As the name is, we find where leading spaces end and trailing spaces start Before calling this,
   * the line should be prepared
   *
    ...
    for (int curr = mm; curr < blocks.size(); curr++) {
      ...
      if (hasVisibleRegion(block.startLine, block.endLine, first, last)) {
        try {
          ...
          float offset1 =
              measureText(
                  lineContent, 0, Math.min(block.endColumn, lineContent.length()), block.endLine);
          ...
          mRect.bottom =
              Math.min(
                  getHeight(),
                  (block.toBottomOfEndLine ? getRowBottom(block.endLine) : getRowTop(block.endLine))
                      - getOffsetY());
  ...
  protected void drawScrollBarVertical(Canvas canvas) {
    ...
    if (mEventHandler.holdVerticalScrollBar() && mDisplayLnPanel) {
      float centerY = topY + length / 2f;
      drawLineInfoPanel(canvas, centerY, getWidth() - mDpUnit * 20);
    }
    ...
    int thumbColor =
        mEventHandler.holdVerticalScrollBar()
            ...
            : mColors.getColor(EditorColorScheme.SCROLL_BAR_THUMB);

    drawColor(canvas, thumbColor, mRect);
  }
  ...
  protected void drawScrollBarHorizontal(Canvas canvas) {
    ...
    drawColor(
        canvas,
        mColors.getColor(
            mEventHandler.holdHorizontalScrollBar()
  ...
  protected void drawDivider(Canvas canvas, float offsetX, int color) {
    ...
    if (shadow) {
      ...
      mPaint.setShadowLayer(Math.min(mDpUnit * 8, getOffsetX()), 0, 0, 0xFF281D1B);
    }
    drawColor(canvas, color, mRect);
    if (shadow) {
      canvas.restore();
  ...
  /**
   * Update the information of cursor Such as the position of cursor on screen(For input method that
   * can go to any position on screen like PC input method)
   *
   ...
   *
   * <p>This is useful when the user uses an input method that does not support the attitude {@link
   * EditorInfo#TYPE_TEXT_FLAG_NO_SUGGESTIONS}
   *
   * <p>Note: Composing texts are marked by an underline. When this switch is set to false, the
   * editor will not provide auto-completion when there is any composing text in editor.
   *
   * <p>This is enabled by default now
   */
   ...
   *
   * <p>If this is set to false, the completion window will never show and auto-completion item
  ...
  /**
   * Sets non-printable painting flags. Specify where they should be drawn.
   *
  ...
  /**
   * Use color {@link EditorColorScheme#TEXT_SELECTED} to draw selected text instead of color
   * specified by its span
   */
  ...
  /**
   * When the parent is a scrollable view group, request it not to allow horizontal scrolling to be
   * intercepted. Until the code cannot scroll horizontally
   *
  ...
  /**
   * Set whether drag mode drag:no fling scroll
   *
  ...
  /**
   * Whether over scroll is permitted. When over scroll is enabled, the user will be able to scroll
   * out of displaying bounds if the user scroll fast enough. This is implemented by {@link
  ...
  /**
   * Get last visible row on screen. The result is <strong>unchecked</strong>. It can be bigger than
   * the max row count in layout
   *
   * @return last visible row
   */
  public int getLastVisibleRow() {
    return Math.max(
        0, Math.min(mLayout.getLayoutHeight(), getOffsetY() + getHeight()) / getRowHeight());
  }
  ...
  /**
   * Move the selection down If the auto complete panel is shown,move the selection in panel to next
   */
  public void moveSelectionDown() {
    if (mLockedSelection == null) {
      ...
      setSelectionRegion(
          mLockedSelection.line,
          mLockedSelection.column,
  ...
  public void moveSelectionRight() {
    if (mLockedSelection == null) {
      ...
      if (line == lineAfter) {
        char ch =
            (columnAfter - 1 < c_column && columnAfter - 1 >= 0)
                ? mText.charAt(lineAfter, columnAfter - 1)
                : '\0';
  ...
  public void moveSelectionHome() {
    if (mLockedSelection == null) {
      ...
      setSelectionRegion(
          mLockedSelection.line, mLockedSelection.column, getSelectingTarget().line, 0, false);
      ensureSelectingTargetVisible();
    }
  }
  ...
  /**
   * @return Text displaying, the result is read-only. You should not make changes to this object as
  ...
  public void setText(@Nullable CharSequence text) {
    ...
    dispatchEvent(
        new ContentChangeEvent(
            ...
            mText
                .getIndexer()
                .getCharPosition(getLineCount() - 1, mText.getColumnCount(getLineCount() - 1)),
            mText));
  ...
  public void setTextSize(float textSize) {
    ...
    setTextSizePx(
        TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, textSize, res.getDisplayMetrics()));
  }
   ...
   *
   * <p>It can be a subclass of {@link EditorColorScheme}. The scheme object can only be applied to
   * one editor instance. Otherwise, an IllegalStateException is thrown.
   *
  ...
  protected ExtractedText extractText(@NonNull ExtractedTextRequest request) {
    ...
    startOffset = 0;
    text.text =
        mConnection.getTextRegion(startOffset, startOffset + request.hintMaxChars, request.flags);
              ...
              for (NewlineHandler handler : handlers) {
                if (handler != null) {
                  if (handler.matchesRequirement(beforeText, afterText)) {
                    try {
                      NewlineHandler.HandleResult result =
                          handler.handleNewline(beforeText, afterText, getTabWidth());
                      if (result != null) {
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    ...
    if (warn) {
      Log.i(
          LOG_TAG,
          "onMeasure():Code editor does not support wrap_content mode when measuring.It will just fill the whole space.");
    }
  ...
  public boolean onGenericMotionEvent(MotionEvent event) {
    if (event.getAction() == MotionEvent.ACTION_SCROLL
      ...
      mEventHandler.onScroll(
          event, event, h_scroll * mVerticalScrollFactor, v_scroll * mVerticalScrollFactor);
      return true;
    }
    ...
    if (isSpanMapPrepared(true, endLine - startLine)) {
      if (startLine == endLine) {
        SpanMapUpdater.shiftSpansOnSingleLineInsert(
            mSpanner.getResult().getSpanMap(), startLine, startColumn, endColumn);
      } else {
        SpanMapUpdater.shiftSpansOnMultiLineInsert(
  ...
  public void afterDelete(
    ...
    if (isSpanMapPrepared(false, endLine - startLine)) {
      if (startLine == endLine) {
        ...
        SpanMapUpdater.shiftSpansOnMultiLineDelete(
            mSpanner.getResult().getSpanMap(), startLine, startColumn, endLine, endColumn);
      }
    }
    ...
    dispatchEvent(
        new ContentChangeEvent(this, ContentChangeEvent.ACTION_DELETE, start, end, deletedContent));
  }
  ...
  public void onAnalyzeDone(TextAnalyzer provider) {
    if (provider == mSpanner) {
      ...
      post(
          () -> {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
              mRenderer.invalidateDirtyRegions(
                  provider.mObjContainer.spanMap, provider.getResult().getSpanMap());
            }
            updateTimestamp();
            invalidate();
          });
  ...
  private class DrawCursorTask {
    ...
    public DrawCursorTask(
        float x, float y, int handleType, SelectionHandleStyle.HandleDescriptor descriptor) {
      this.x = x;
      this.y = y;
    ...
    void execute(Canvas canvas) {
      ...
      if (!descriptor.position.isEmpty()) {
        if ((mEventHandler.holdInsertHandle()
            ...
            || (mEventHandler.mSelHandleType == EditorTouchEventHandler.SelectionHandle.LEFT
                && handleType == SelectionHandleStyle.HANDLE_TYPE_LEFT)
            || (mEventHandler.mSelHandleType == EditorTouchEventHandler.SelectionHandle.RIGHT
                && handleType == SelectionHandleStyle.HANDLE_TYPE_RIGHT)) {
          x =
              mEventHandler.mMotionX
                  + (descriptor.alignment != SelectionHandleStyle.ALIGN_CENTER
                          ? descriptor.position.width()
                          : 0)
                      * (descriptor.alignment == SelectionHandleStyle.ALIGN_LEFT ? 1 : -1);
          y = mEventHandler.mMotionY - descriptor.position.height() * 2 / 3f;
        }
      }

      if (handleType != SelectionHandleStyle.HANDLE_TYPE_UNDEFINED
          || mCursorBlink.visibility
          || mEventHandler.holdInsertHandle()) {
        mRect.top = y - getRowHeight();
        mRect.bottom = y;
        mRect.left = x - mInsertSelWidth / 2f;
        mRect.right = x + mInsertSelWidth / 2f;
  ...
  public boolean getIsBlockLineRpg() {
    return this.isBlockLineRpg;
  }

  public void setIsBlockLineRpg(boolean isBlockLineRpg) {
    this.isBlockLineRpg = isBlockLineRpg;
  }
}
