...
class Magnifier {
  ...
  public Magnifier(CodeEditor editor) {
    ...
    maxTextSize =
        TypedValue.applyDimension(
            TypedValue.COMPLEX_UNIT_SP, 100, view.getResources().getDisplayMetrics());
    scaleFactor = 1.4f;
    paint = new Paint();
  }
   ...
   *
   * <p>This should be called when new content has been drawn on the target view so that the content
   * in magnifier will not be invalid.
   *
      ...
      PixelCopy.request(
          ...
          (var statusCode) -> {
            ...
            if (statusCode == PixelCopy.SUCCESS) {
              ...
              var scaled =
                  Bitmap.createScaledBitmap(clip, popup.getWidth(), popup.getHeight(), false);
              clip.recycle();
   ...
   * {@link CodeEditor#drawView(Canvas)}
   */
