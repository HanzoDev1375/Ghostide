...
public class IconSpanManager {
  ...
  private void sortIconSpans() {
    iconSpans.sort(
        (a, b) -> {
          if (a.getLine() != b.getLine()) return Integer.compare(a.getLine(), b.getLine());
  ...
  public List<IconSpan> getIconSpansForLine(int line) {
    ...
    for (IconSpan span : iconSpans) {
      if (span.getLine() == line) result.add(span);
    }
    return result;
  }

  // IconSpanManager.java - تغییر در drawIconSpans
  public void drawIconSpans(Canvas canvas) {
    ...
    var textPaint = editor.getTextPaint();
    for (IconSpan span : iconSpans) {
      try {
        ...
        if (iconSize <= 0) iconSize = (int) (rowHeight * 0.6f);
        ...
        if (actualColumn < columnCount) {
          char c = contentLine.charAt(actualColumn);
          charWidth = textPaint.measureText(String.valueOf(c));
        }

        float spaceWidth = textPaint.measureText(" ");

        if (span.isRightAligned()) {
          iconX = charX + charWidth + spaceWidth;
        } else {
          iconX = charX - iconSize - spaceWidth;
          if (iconX < spaceWidth) {
            iconX = charX + charWidth + spaceWidth;
          }
        }
        ...
        if (span.shouldDrawBackground() && span.getBackgroundColor() != 0) {
          int oldColor = iconPaint.getColor();
          Paint.Style oldStyle = iconPaint.getStyle();
          iconPaint.setColor(span.getBackgroundColor());
          iconPaint.setStyle(Paint.Style.FILL);
          float radius = editor.getDpUnit() * 6;
          canvas.drawRoundRect(
              ...
              radius,
              iconPaint);
          iconPaint.setColor(oldColor);
          iconPaint.setStyle(oldStyle);
        }
