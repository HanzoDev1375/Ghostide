...
public class ColorPickerView extends View {
    ...
    private void initWith(Context context, AttributeSet attrs) {
        final TypedArray typedArray =
                context.obtainStyledAttributes(attrs, R.styleable.ColorPickerPreference);
        ...
        pickerColorEditTextColor =
                typedArray.getInt(R.styleable.ColorPickerPreference_pickerColorEditTextColor, 0xffffffff);

        WHEEL_TYPE wheelType =
                WHEEL_TYPE.indexOf(typedArray.getInt(R.styleable.ColorPickerPreference_wheelType, 0));
        ColorWheelRenderer renderer = ColorWheelRendererBuilder.getRenderer(wheelType);

        alphaSliderViewId =
                typedArray.getResourceId(R.styleable.ColorPickerPreference_alphaSliderView, 0);
        lightnessSliderViewId =
                typedArray.getResourceId(R.styleable.ColorPickerPreference_lightnessSliderView, 0);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int width = 0;
        if (widthMode == MeasureSpec.UNSPECIFIED) width = widthMeasureSpec;
        else if (widthMode == MeasureSpec.AT_MOST) width = MeasureSpec.getSize(widthMeasureSpec);
        else if (widthMode == MeasureSpec.EXACTLY) width = MeasureSpec.getSize(widthMeasureSpec);

        int heightMode = MeasureSpec.getMode(heightMeasureSpec);
        int height = 0;
        if (heightMode == MeasureSpec.UNSPECIFIED) height = heightMeasureSpec;
        else if (heightMode == MeasureSpec.AT_MOST) height = MeasureSpec.getSize(heightMeasureSpec);
        else if (heightMode == MeasureSpec.EXACTLY) height = MeasureSpec.getSize(heightMeasureSpec);
        int squareDimen = width;
        if (height < width) squareDimen = height;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (colorWheel != null && currentColorCircle != null) {
            ...
            currentColorCanvas.drawCircle(
                    currentColorCircle.getX(), currentColorCircle.getY(), size + 4, alphaPatternPaint);
            currentColorCanvas.drawCircle(
                    currentColorCircle.getX(), currentColorCircle.getY(), size + 4, colorWheelFill);

            selectorStroke =
                    PaintBuilder.newPaint()
                            .color(0xffffffff)
                            .style(Paint.Style.STROKE)
                            .stroke(size * (STROKE_RATIO - 1))
            ...
            if (showBorder)
                colorWheelCanvas.drawCircle(
                        ...
                        size + (selectorStroke.getStrokeWidth() / 2f),
                        selectorStroke);
    ...
    public void setSelectedColor(int previewNumber) {
        if (initialColors == null || initialColors.length < previewNumber) return;
    ...
    public void setInitialColor(int color, boolean updateText) {
        ...
        if (this.colorEdit != null && updateText) setColorText(color);
    ...
    public void setAlphaValue(float alpha) {
        ...
        this.initialColor =
                Color.HSVToColor(
                        Utils.alphaValueAsInt(this.alpha),
                        currentColorCircle.getHsvWithLightness(this.lightness));
        if (this.colorEdit != null)
            this.colorEdit.setText(Utils.getHexString(this.initialColor, this.alphaSlider != null));
    ...
    public void setColorEditTextColor(int argb) {
        ...
        if (colorEdit != null) colorEdit.setTextColor(argb);
    }
    ...
    private void setColorPreviewColor(int newColor) {
        if (colorPreview == null
                || initialColors == null
                || colorSelection > initialColors.length
                || initialColors[colorSelection] == null) return;
