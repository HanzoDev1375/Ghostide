 ...
 * #load(DocumentSource, String, OnLoadCompleteListener, OnErrorListener, int[])}. In this
...
public class PDFView extends RelativeLayout {
    ...
    /**
     * The same pages but with a filter to avoid repetition (ex: 0, 2, 8, 1)
     */
    private int[] filteredUserPages;
    /**
     * The same pages but with a filter to avoid repetition (ex: 0, 1, 1, 2, 2, 3, 3, 3)
     */
        ...
        if (swipeVertical) {
            ...
            if (direction < 0 && currentYOffset < 0) {
                return true;
                    ...
                    && currentYOffset + toCurrentScale(optimalPageHeight) > getHeight()) {
                return true;
            }
        }
        return false;
    ...
    private void drawWithListener(Canvas canvas, int page, OnDrawListener listener) {
        if (listener != null) {
            ...
            listener.onLayerDrawn(
                    canvas, toCurrentScale(optimalPageWidth), toCurrentScale(optimalPageHeight), page);
    ...
    void loadComplete(PdfDocument pdfDocument, int pageWidth, int pageHeight) {
        ...
        renderingHandler =
                new RenderingHandler(renderingHandlerThread.getLooper(), this, pdfiumCore, pdfDocument);
    ...
    /**
     * Calculate the x/y-offset needed to have the given page centered on the screen. It doesn't take
            ...
            PDFView.this.post(
                    new Runnable() {
                        ...
                        public void run() {
                            if (pageNumbers != null) {
                                PDFView.this.load(
                                        documentSource, password, onLoadCompleteListener, onErrorListener, pageNumbers);
                            } else {
                                PDFView.this.load(
                                        documentSource, password, onLoadCompleteListener, onErrorListener);
                            }
                        }
                    });
        }
