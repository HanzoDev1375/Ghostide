...
class XmlLayoutResourceFinder(private val context: Context, resDirPath: String?) {
  ...
  fun findStyleAttributeValue(style: String, property: XmlLayoutProperties.PropertySpec): String? {
  ...
  private fun findStyleAttributeValue(
    ...
    for (sdk in Build.VERSION.SDK_INT downTo 0) {
      ...
      if (attrs != null) {
        return attrs[property.attrName]
                ?: return findStyleAttributeValue(
                        styleParents?.get(sdk)?.get(style),
                        property,
                        visitedStyles
                )
      }
  ...
  fun findUserDrawable(resName: String?): Drawable? {
    if (!(resourcesDir == null || resName == null || !resName.startsWith("@drawable/"))) {
      ...
      val drawableNine = loadNinePatchFile(File(File(resourcesDir, "drawable"), "$resName2.9.png"))
      ...
      for (i in 0 until lenDpi) {
        val drawable =
                loadImageFile(File(File(resourcesDir, "drawable-" + arrDpi[i]), "$resName2.png"))
        if (drawable != null) {
          return drawable
        }
      }
      for (i in 0 until lenDpi) {
        val drawable =
                loadImageFile(File(File(resourcesDir, "drawable-" + arrDpi[i]), "$resName2.jpg"))
        if (drawable != null) {
          return drawable
        }
      }
      for (i in 0 until lenDpi) {
        val drawable =
                loadNinePatchFile(
                        File(File(resourcesDir, "drawable-" + arrDpi[i]), "$resName2.9.png")
                )
        if (drawable != null) {
          return drawable
        }
        ...
        resourcesDir.listFiles()?.forEach { drawableDir ->
          if (drawableDir.isDirectory && drawableDir.name.startsWith("drawable")) {
            drawableDir.listFiles()?.forEach { drawableFile ->
              ...
              if (name.lowercase(Locale.getDefault()).endsWith(".png") ||
                              name.lowercase(Locale.getDefault()).endsWith(".jpg") ||
                              name.lowercase(Locale.getDefault()).endsWith(".xml")
              ) {
  ...
  fun getBaseStyle(style: String?): String? {
    ...
    return if (style.startsWith("@style/"))
            getBaseStyle(style.substring("@style/".length), HashSet())
    else style
  }
