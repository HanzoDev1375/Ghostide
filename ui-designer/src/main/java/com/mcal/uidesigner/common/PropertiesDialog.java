...
public class PropertiesDialog extends MessageBox {
  ...
  private static class PropertyCommandEntryAdapter extends ArrayAdapter<PropertyCommand> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
      ...
      if (view == null) {
        view =
            LayoutInflater.from(getContext()).inflate(R.layout.propertydialog_entry, parent, false);
      }
      PropertyCommand command = getItem(position);
      ((TextView) view.findViewById(R.id.widgetmenuEntryName))
          .setText(Html.fromHtml(command.getName()));
      ((ImageView) view.findViewById(R.id.widgetmenuEntryImage))
          .setImageResource(
              AndroidHelper.obtainImageResourceId(getContext(), command.getIconAttr()));
      ...
      if (helpUrl != null) {
        helpView.setOnClickListener(
            v -> ((XmlLayoutDesignActivity) getContext()).showHelp(helpUrl));
      }
      return view;
    }
  }
