...
public class ProxyTextView {
  ...
  public void setTextAppearance(String value) {
    try {
      if (value.startsWith("?android:attr/")) {
        int attrID =
            (Integer)
                android.R.attr.class.getField(value.substring("?android:attr/".length())).get(null);
        ...
        textView.setTextAppearance(
            ...
            (Integer)
                R.style
                    .class
                    .getField(value.substring("@android:style/".length()).replace(".", "_"))
                    .get(null));
      }
    } catch (Throwable th) {
      th.printStackTrace();
  ...
  private void updateFont() {
    ...
    if (this.fontFamily == null
        || (tf = Typeface.create(this.fontFamily, this.textStyle)) == null) {
