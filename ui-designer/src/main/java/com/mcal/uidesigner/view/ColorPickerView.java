...
public class ColorPickerView extends View {
  ...
  private void init() {
    ...
    scale =
        c.getResources().getDisplayMetrics().density
            * (Math.min(300.0f, AndroidHelper.getScreenSizeDip(c) - 150.0f) / 256.0f);
    paint = new Paint(1);
  ...
  private void getCurrentXY() {
    ...
    for (int x = 0; x < 256; x++) {
      for (int y = 0; y < 256; y++) {
        int dist =
            getColorDist(getDerivedColorForXY(x, y), currentColor | ViewCompat.MEASURED_STATE_MASK);
        if (dist == 0) {
          currentX = x;
          currentY = y;
          return;
    ...
    for (int y = 0; y < 256; y++) {
      for (int x = 0; x < 256; x++) {
        if (y == 0) {
          derivedColors[index] =
              Color.rgb(
                  255 - (((255 - Color.red(mainColor)) * x) / MotionEventCompat.ACTION_MASK),
                  255 - (((255 - Color.green(mainColor)) * x) / MotionEventCompat.ACTION_MASK),
                  255 - (((255 - Color.blue(mainColor)) * x) / MotionEventCompat.ACTION_MASK));
          topColors[x] = derivedColors[index];
        } else {
          derivedColors[index] =
              Color.rgb(
                  ((255 - y) * Color.red(topColors[x])) / MotionEventCompat.ACTION_MASK,
                  ((255 - y) * Color.green(topColors[x])) / MotionEventCompat.ACTION_MASK,
                  ((255 - y) * Color.blue(topColors[x])) / MotionEventCompat.ACTION_MASK);
        }
        index++;
      }
    }
  ...
  protected void onDraw(Canvas canvas) {
    ...
    for (int i = 0; i < 256; i++) {
      ...
      canvas.drawLine(
          ((float) (i + 10)) * scale, 0.0f, ((float) (i + 10)) * scale, 40.0f * scale, paint);
    }
    paint.setStrokeWidth(3.0f * scale);
    paint.setColor(ViewCompat.MEASURED_STATE_MASK);
    canvas.drawLine(
        ((float) (translatedHue + 10)) * scale,
        0.0f,
        ((float) (translatedHue + 10)) * scale,
        40.0f * scale,
        paint);
    ...
    for (int i2 = 0; i2 < 256; i2++) {
      paint.setShader(
          new LinearGradient(
              ...
              0.0f,
              306.0f * scale,
              new int[] {derivedColors[i2], ViewCompat.MEASURED_STATE_MASK},
              null,
    ...
    paint.setStrokeWidth(2.0f * scale);
    canvas.drawCircle(
        ((float) (currentX + 10)) * scale, ((float) (currentY + 50)) * scale, 6.0f * scale, paint);
    paint.setStyle(Paint.Style.STROKE);
    paint.setColor(ViewCompat.MEASURED_STATE_MASK);
    paint.setStrokeWidth(1.0f);
  ...
  protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    setMeasuredDimension((int) (100.0f * scale), (int) (266.0f * scale));
  }
  ...
  public boolean onTouchEvent(@NonNull MotionEvent event) {
    if (event.getAction() == 0 || event.getAction() == 2) {
      ...
      currentColor =
          Color.argb(
              Color.alpha(currentColor), Color.red(color), Color.green(color), Color.blue(color));
      listener.colorChanged(currentColor, toHexColor(currentColor));
      invalidate();
    }
    return true;
